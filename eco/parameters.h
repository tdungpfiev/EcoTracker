...
// Conjugate Gradient Options Structure =====================================
struct CgOpts
{
    bool debug;
    bool CG_use_FR;
...
//    IcFeatures ic_features;
    ...
    int newton_iterations = 1;	 // The number of Newton iterations used for optimizing the detection score
    ...
    int train_gap = 10; // The number of intermediate frames with no training(0 corresponds to training every frame)
    int skip_after_frame = 20; // After which frame number the sparse update scheme should start(1 is directly)
