...
//    printf("resize_factor: %f, df: %d,sample_sz: %f x %f,input_sz: % f x % f \r\n ",
//          resize_factor, df,
...
//          input_sz.width, input_sz.height);
    ...
    sample_sz.width = round(sample_sz.width)>2.0?round(sample_sz.width):2.0;
    sample_sz.height = round(sample_sz.height)>2.0?round(sample_sz.height):2.0;
...
vector<cv::Mat> FeatureExtractor::get_hog_features_simd(const vector<cv::Mat> ims)
{
	...
	for (unsigned int k = 0; k < ims.size(); k++)
	{
        ...
        binSize = params_.hog_features.fparams.cell_size; //4;
		...
		float clipHog = 0.2f;
		float *I, *M, *O, *H;
                I = (float *)malloc(h * w * d*sizeof(float));
                M = (float *)malloc(h * w*sizeof(float));
                O = (float *)malloc(h * w*sizeof(float));
                H = (float *)malloc(hb * wb * nDim*sizeof(float));
		/*
		cv::Mat ims_f;
