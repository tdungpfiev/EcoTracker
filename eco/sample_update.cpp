...
namespace eco
{
void SampleUpdate::init(const vector<cv::Size> &filter,
...
void SampleUpdate::update_sample_space_model(const ECO_FEATS &new_train_sample)
{
    ...
    //std::cout << "gram_vector \r\n ";
    //std::cout << gram_vector << std::endl;
    float new_train_sample_norm = 2 * FeatureComputeEnergy(new_train_sample); //2(c^2+d^2)
    //std::cout << " new_train_sample_norm "  << new_train_sample_norm << std::endl;
...
//        for(int p = 0; p< prior_weights_.size() ; p++)
//        {
//            //printf("value prior_weights at %d : %d \r\n ", prior_weights_[p]);
//        }
        // update sample space
