...
float* hogNormMatrix( float *H, int nOrients, int hb, int wb, int bin ) {
  ...
  N = (float*)malloc(hb1*wb1*sizeof(float)); N1=N+hb1+1;
  ...
  int nOrients, int softBin, bool full, float clip )
{
    ...
    R = (float*)malloc(wb*hb*nOrients*sizeof(float));
