...
//    }
//	printMat(im);
//    //printf("is_color_image_: %d, rect: %f, %f, %f, %f", is_color_image_, rect.x, rect.y, rect.width, rect.height);

    // Get the init position
    pos_.x = rect.x + (rect.width - 1.0) / 2.0;
    pos_.y = rect.y + (rect.height - 1.0) / 2.0;
//    //printf("pos_:%f, %f", pos_.y, pos_.x);
...
//    //printf("search_area:%f \r\n ", search_area);
    ...
    //printf("currentscale:%f", currentScaleFactor_);

    // target size at the initial scale
    base_target_size_ = cv::Size2f(rect.size().width / currentScaleFactor_, rect.size().height / currentScaleFactor_);
//    //printf("base_target_size_:%f x %f \r\n ", base_target_size_.height, base_target_size_.width);
...
//    {
////        int padw = rect.width * params_.padding;
////        int padh = rect.height * params_.padding;
////        if(padw >= padh)
////        {
////            currentScaleFactor_ = padw / (float) 150;
////        } else
////        {
////            currentScaleFactor_ = padh / (float) 150;
////        }
//        img_sample_size_.width = base_target_size_.width * params_.padding;

//        img_sample_size_.height = base_target_size_.height * params_.padding;
//        img_sample_size_.width = (((int)(img_sample_size_.width /(2 * params_.hog_features.fparams.cell_size))) * 2 * params_.hog_features.fparams.cell_size)
//                + params_.hog_features.fparams.cell_size * 2;
//        img_sample_size_.height = (((int) (img_sample_size_.height / (2 * params_.hog_features.fparams.cell_size))) * 2 * params_.hog_features.fparams.cell_size)
//                + params_.hog_features.fparams.cell_size * 2;
////        base_target_size_ = cv::Size2f(rect.size().width/ currentScaleFactor_, rect.size().height / currentScaleFactor_);
...
//    {
//    }
//    //printf("img_sample_size_: %d x %d", img_sample_size_.height, img_sample_size_.width);
////    img_sample_size_ = cv::Size2i(150 , 150);
    ...
    for (size_t i = 0; i != feature_size_.size(); ++i)
    {
        ...
        //printf("filter_size_ %lu: %d x %d", i, filter_size_[i].height, filter_size_[i].width);
    ...
    //printf("output_index_:%lu, output_size_:%lu", output_index_, output_size_);

    //Compute the 2d Fourier series indices by kx and ky.
    for (size_t i = 0; i < filter_size_.size(); ++i) // for each filter
    {
        cv::Mat_<float> tempy(filter_size_[i].height + 1 , 1, CV_32FC1);
        ...
        kx_.push_back(tempx);
        //printf("For filter i: %lu, N: %d, ky:%d x %d, kx:%d x %d", i,
              //filter_size_[i].height,
              //ky_[i].size().height, ky_[i].size().width,
              //kx_[i].size().height, kx_[i].size().width);
        //std::cout << "ky_"  << ky_[i] << std::endl;
        //std::cout << "kx_"  << kx_[i] << std::endl;
    }
...
//        printMat(temp_f);
        ...
        //printf("reg_energy_ %lu: %f", i, energy);
    }
...
//    {
        ...
        //printf("scale: min:%d, max:%d", scalemin, scalemax);
        //printf("scale_factors_:");
...
//    }
    ...
    //printf("scalefactor min: %f max: %f", params_.min_scale_factor, params_.max_scale_factor);
    ...
    //printf("xl size: %lu, %lu, %d x %d \r\n ", xl.size(), xl[0].size(), xl[0][0].rows, xl[0][0].cols);
    ...
    //{
        //printf("xlf feature %lu 's size: %lu, %d x %d \r\n", i, xlf[i].size(), xlf[i][0].rows, xlf[i][0].cols);
    //}
    ...
    //{
        //printf("projection_matrix %lu 's size: %d x %d \r\n", i, projection_matrix_[i].rows, projection_matrix_[i].cols);
    //}
    ...
    //for (size_t i = 0; i < xlf.size(); i++)
    //{
        //printf("xlf_proj feature %lu 's size: %lu, %d x %d \r\n", i, xlf_proj[i].size(), xlf_proj[i][0].rows, xlf_proj[i][0].cols);
    //}
...
//    {
//        //printf("sample_energy_: %lu, %lu, %d x %d \r\n", i, sample_energy_[i].size(), sample_energy_[i][0].rows, sample_energy_[i][0].cols);
//    }
//    for (size_t i = 0; i < proj_energy.size(); i++)
//    {
//        //printf("proj_energy: %lu, %lu, %d x %d \r\n", i, proj_energy.size(), proj_energy[i].rows, proj_energy[i].cols);
//    }
...
//    {
//        //printf("hf: %lu, %lu, %d x %d \r\n", i, hf[i].size(), hf[i][0].rows, hf[i][0].cols);
//    }
...
//            //std::cout << "============================================================ \r\n";
//            //std::cout <<  scores_fs_sum[k1] << std::endl;
        }
    }
    ...
    //printf(" image_support_size [%d]: \r\n ", img_support_size_.width);
    //printf(" output_zize [%d]: \r\n ", output_size_);
    //printf(" disp.col [%f]: \r\n", scores.get_disp_col());
    //printf(" disp.row [%f]: \r\n", scores.get_disp_row());
    ...
    //printf(" Value of output scale: %f x %f x %f \r\n ", scale_factors_[scale_change_factor], dx, dy);
...
//    for (size_t i = 0; i < feature_size_.size(); i++)
//    {
//        //printf("features %lu: %d %d %d x %d", i, feature_dim_[i], compressed_dim_[i], feature_size_[i].height, feature_size_[i].width);
//    }
}
